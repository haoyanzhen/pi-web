...
function filePathFromSegments(segments: string[]): string {
  ...
  if (slashJoined === "~" || slashJoined.startsWith("~/")) {
    return path.join(homedir(), slashJoined.slice(2));
  }
...
type MammothModule = {
  convertToHtml: (
    input: { path: string },
    options: { externalFileAccess: false; convertImage: unknown }
  ) => Promise<{ value: string }>;
  images: { dataUri: unknown };
};

async function loadOptionalMammoth(): Promise<MammothModule | null> {
  try {
    const dynamicImport = new Function("specifier", "return import(specifier)") as (specifier: string) => Promise<MammothModule>;
    return await dynamicImport("mammoth");
  } catch {
    return null;
  }
}
...
export async function GET(
  ...
  try {
    ...
    if (type === "preview") {
      ...
      const mammoth = await loadOptionalMammoth();
      if (!mammoth) {
        return NextResponse.json({ error: "DOCX preview unavailable: mammoth is not installed" }, { status: 501 });
      }
