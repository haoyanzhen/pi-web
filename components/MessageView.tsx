...
type MermaidApi = {
  ...
  parse: (code: string, options?: Record<string, unknown>) => Promise<boolean> | boolean;
  render: (id: string, code: string) => Promise<{ svg: string }>;
};

async function loadOptionalMermaid(): Promise<MermaidApi | null> {
  try {
    const dynamicImport = new Function("specifier", "return import(specifier)") as (specifier: string) => Promise<{ default?: MermaidApi } & MermaidApi>;
...
function MermaidBlock({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  useEffect(() => {
    ...
    const render = async () => {
      const mermaid = await loadOptionalMermaid();
      if (!mermaid) {
  ...
  const body =
    unavailable ? (
      <div className="mermaid-block mermaid-block-error">Mermaid preview unavailable</div>
